...
fase del tipo PD. Éste método fue desarrollado por el Grupo de Óptica Aplicada en
...
combinado\footnote{Se espera que por medio de un convenio de colaboración con
...
\begin{equation}
u_{obj} = \exp{\left(\frac{-r^2}{w^2(z)}\right)}.
\label{eq:field_at_input}
\end{equation}
 Donde conocemos o podemos aprocimar el ancho del haz $w(z)$.
 La Eq.~\ref{eq:field_at_input} describe entonces el campo óptico en el plano objeto, es decir, a una
...
\begin{figure}[h!]
\centering
\includegraphics[scale=1]{exp_res_correction_1.pdf}
