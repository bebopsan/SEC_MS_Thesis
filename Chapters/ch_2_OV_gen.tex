...
estados el error relativo promedio es menor al $1\%$.  Se
calcula el error relativo porcentual promedio para cada estado como: 
\begin{equation*}
error = \left(\sum_{n=1}^{52}\frac{|I_n^e-I_n^s|}{I_n^e}\right) \times \frac{100}{52}.
\end{equation*}   
...
Fig. \ref{fig:Ma_caracterization_results} y el resto de medidas se pueden
observar en la siguiente página web:\\
\hspace*{\fill}\href{http://goo.gl/69kSu2}{\textbf{http://goo.gl/69kSu2}}. \hspace*{\fill}\\
...
los estados usados para la construcción de las matrices. 

Con el fin de obtener un modelo más preciso, y que solucionara los
vacíos del que hemos presentado, propusimos un método de
caracterización basado en ajuste numérico de parámetros. El ajuste se
logra por medio de rutinas de minimización que actúan sobre un
funcional propuesto por nosotros que se expone en la sección
siguiente.   
\section{Un método numérico basado en minimización de parámetros}
Encontrar los números complejos $A$ y $B$ (o reales $X$, $Y$, $Z$,
$W$) necesarios para ensamblar la
matriz de Jones puede ser implementado como un problema de polarimetría que requiere de
...
Tras implementar el método de caracterización basados en un modelos de
caja negra de \citetChGen{Ma2010}, y realizar nuestro propio método concluimos que
...
\subsection{Función a minimizar}
