...
cuando el ángulo de inclinación de las moléculas es de 90º. La
...
el PSG genera un estado de polarización a la entrada horizontal y el
PSD detecta un estado vertical. Como es de esperarse, en esta
...
óptimo]{Medida de la intensidad normalizada del SLM Holoeye LC-2002 en la configuración típica
  ...
  polarización a la entrada horizontal y el PSD detecta un estado vertical.}
\label{fig:amp_H_V_SLM_2002}
\end{figure}
Este tipo de modulaciónes
sirven para proyectar imágenes en niveles de gris y se pueden lograr
con dos polarizadores como PSG y PSD que generen estados ortogonales
entre si.
...
La curva anterior caracteriza la modulación de amplitud del SLM para
...
distintos y medir la modulación hasta encontrar un estado con las
condicienes adecuadas. Hacer esto experimentalmente es un proceso
engorroso y requiere de mucho tiempo porque hay infinidad de
combinaciones posibles. 
En cambio, vamos a medir experimentalmente sólo 12 combinaciones de PSG y PSD y con
...
de parámetros.
% basado en un algorítmo de minimización basado en la búsqueda del
% gradiente. 
Una vez conocido el modelo del SLM se puede buscar el
estado que cumple las condiciones haciendo experimentos simulados con
los cuales si es práctico hacer una búsqueda entre múltiples estados.
...
\mathbf{HWP}\right)\ In.
\end{equation}
Y por consigiente el PSD correspondiente a un PSG dado es el Bra,
o el vector que es complejo conjugado del Ket.
\begin{equation}
<J|= |J>^{\dagger} = \begin{pmatrix} J_x &  J_y\end{pmatrix} 
\end{equation}
Por otra parte, la matriz de Jones del SLM cumple el papel que tendría un
operador en la mecánica cuántica. Es decir que si los Bra y Ket representan vectores normalizados, la
...
Los estados que determinan una matriz 
...
Y medir la modulación de fase implica medir cómo cambia la fase del
frente de onda. 
