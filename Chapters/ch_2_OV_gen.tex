...
polarización]{Esquema general de un sistema óptico para
  caracterización de modulación de intensidad de un TN-SLM. El ángulo
  de los elementos ópticos se toma a partir del eje $x$ y se mide con
  respecto al eje rápido de las placas retardadoras que está señalado
...
ortogonal ($\hat{I}^{\perp} $) es entonces un complemento de la original y su suma debe ser
...
normalizado ($\hat{I}$) y que es equivalente a aquellos obtenidos usando el producto
...
calcula el error absoluto promedio para cada estado como el valor
absoluto entre las intensidades normalizadas (con valor de 0 a 1)
simulada $I_n^s$ y
experimental $I_n^e$ para cada nivel de gris,
\begin{equation*}
...
elípticos\footnote{Para saber cómo ubicar los
...
\hspace*{\fill}
\href{https://github.com/bebopsan/Ellipsometry\_for\_dummies.git}{\textbf{https://github.com/bebopsan/Ellipsometry\_for\_dummies.git}}\hspace*{\fill}.} ortonormales no usados
...
Aún cuando la imprecisión del método se hace evidente al observar las
curvas de la Fig.~\ref{fig:Ma_caracterization_results}, se midió un error promedio global para los
100 estados como medida cuantitativa, $$error_g =
\sum_{i=1}^{100}\frac{error_i}{100},$$ y se obtuvo un valor de $error_g
= 0.09$. Es decir que con intensidades normalizadas la diferencia
entre la medida y la simulación es de alrededor de $9\%$.
...
=0.08$, que es un poco menor que el anterior. Sin embargo, sigue ocurriendo que la exactitud se pierde a partir del
...
significativa con un error absoluto global de $error_g= 0.02$ que
es 4.5 veces menor que el obtenido con el método de Ma et al. No
...
propuesta para esta aplicación por colegas del Centro de
...
%Como se mencionó en la Sección \ref{sec:ChGen_marco_teorico}, 
Para generar un haz \textit{similar}\footnote{Decimos que el haz con frente de onda
helicoidal generado por medio de este tipo de máscaras es
\textit{similar} a los que se describen con la
Eq.~\ref{eq:Laguerre-Gauss} porque los cambios súbitos de fase presentes en las
máscaras introducen efectos de difracción que se describen mejor con
combinaciones de polinomios de Bessel o de Kummer como han mostrado
\citetChGen{Berry2004, Bekshaev2008a}.} a los haces Laguerre-Gauss producidos en
cavidades perfectamente cilíndricas sólo basta propagar un haz
gausiano por una máscara de fase espiral como la de la
Fig.~\ref{fig:mask_app} para que adquiera las distribuciones de fase de la
Fig.~\ref{fig:spirals}. 
Haciendo esto, los perfiles de
...
al brazo objeto del montaje de la Fig.~\ref{fig:mach_zehnder}.  
...
y fase desarrollados durante el transcurso de este proyecto. \footnote{Esta aplicacion
...
astigmatismo, muy similar al comportamiento de las reportadas por
\citetChGen{Singh2008}. Por otra parte, creemos que la no uniformidad de la
intensidad en el anillo se debe a que la modulación de amplitud no es
constante para todos los niveles de gris. Una modulación de amplitud
no lineal o no constante podría hacer que píxeles con niveles de gris
distintos modificaran la polarización de tal manera que el PSD
redujera la intensidad de ciertos segmentos del anillo de luz como se
ve en el VO de carga $l=1$ en la Fig.~\ref{fig:VOs_I10}. 
...
permite sacar el haz de eje. Sin embargo, haciendo una revisión del
estado del arte se puede observar que el uso de esta configuración es
muy reducido pues sacar los VOs de eje usando una red de difracción
...
significativo de ordenes de difracción en la luz que atraviesa el
SLM. Si estos ordenes se 
...
y en la Fig.~\ref{fig:VOs_I6}. Es por esto que autores clásicos y modernos del
estudio de VOs como 
\citetChGen{Bazhenov1990}, \citetChGen{Leach2004,Leach2005}, y
...
dislocaciones en su centro\footnote{Otras publicaciones recientes en las cuales
  se producen VOs fuera de eje son \citepChGen{Curtis2003,Frhapter2005,Arias2013,Novoa2014}.}. 
Las máscaras del tipo tenedor resultan de la combinación de una
...
los otros ordenes, esto implica una menor eficiencia y la necesidad de
...
componentes. Además, para evitar aberraciones horizontales que
aumentan con la distancia de propagación y que se pueden deber a la
inclinación del haz con respecto a la cámara, es recomendable observar
los VOs fuera de eje en un plano focal y bloquear la luz procedente de
otros ordenes de difracción.
...
\ref{fig:zoom_vortex} dónde se hace evidente que hay al menos una
componente de astigmatismo. 

\begin{figure}[h!]
\centering
\includegraphics[scale=0.6]{zoom_vortex_l2.pdf}
\caption[Magnificación de un VO dónde las aberraciones
parecen haber separado la singularidad de carga $l=2$ en dos
singularidades de carga $l=1$.]{Magnificación de uno de los VOs de la
  Fig.~\ref{fig:diffracted_OV_I6_and_I10} dónde las
  aberraciones parecen haber separado la singularidad de carga $l=2$ en dos
singularidades de carga $l=1$.}
\label{fig:zoom_vortex}
\end{figure}
